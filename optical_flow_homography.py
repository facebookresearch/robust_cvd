...
def parse_args():
    ...
    parser.add_argument("--model", choices=["raft"], default="raft")
    parser.add_argument("--pretrained_weights", nargs="+")
...
def infer(args, OFlowModel, device, img1_name, img2_name):
    ...
    if args.homography != 1:
        with torch.no_grad():
            if args.model == "raft":
                # Compute flows
                flow_low, flow_up = OFlowModel(img1, img2, iters=20, test_mode=True)
                flow = flow_up[0].permute(1, 2, 0).data.cpu().numpy()
            else:
                raise ValueError("False flow model specified.")

    else:
        with torch.no_grad():
            if args.model == "raft":
                # Compute flows
                flow_low, flow_up = OFlowModel(img1, img2_reg, iters=20, test_mode=True)
                flow = flow_up[0].permute(1, 2, 0).data.cpu().numpy()
            else:
                raise ValueError("False flow model specified.")
...
def process(args):
    ...
    if args.model == "raft":
        OFlowModel = torch.nn.DataParallel(RAFT(args))
        OFlowModel.load_state_dict(
            torch.load(args.pretrained_weights, map_location=DEVICE)
        )
        OFlowModel = OFlowModel.module
        OFlowModel.to(DEVICE)
        OFlowModel.eval()
    else:
        # Has been checked by argparse choices, just an extra test here.
        raise ValueError(
            "Specified model {args.model} not supported. Use 'raft' for args.model instead."
        )
